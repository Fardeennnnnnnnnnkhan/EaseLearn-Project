...
const Header = ({ isAuth }) => {
  ...
  return (
    <div
      className={`w-full fixed top-0 left-0 flex justify-between items-center px-10 py-6 transition-all duration-300 z-50 backdrop-blur-md ${
    isScrolled ? "bg-black/30 shadow-lg" : "bg-black bg-gradient-to-t from-teal-900/40 to-black"
  }`}
    >
      ...
      <div className="left hidden sm:block text-lg tracking-tighter">
        <div className="links flex gap-6">
          <NavLink className="text-xl text-white font-medium transition-colors hover:underline duration-200 hover:text-teal-400" to="/">
            Home
          </NavLink>
          <NavLink className="text-xl text-white font-medium transition-colors hover:underline duration-200 hover:text-teal-400" to="/courses">
            Courses
          </NavLink>
          <NavLink className="text-xl text-white font-medium transition-colors hover:underline duration-200 hover:text-teal-400" to="/about">
            About
          </NavLink>
          {isAuth ? (
            <NavLink className="text-xl text-white font-medium transition-colors hover:underline duration-200 hover:text-teal-400" to="/account">
              Account
            </NavLink>
          ) : (
            <NavLink className="text-xl  text-white font-medium transition-colors hover:underline duration-200 hover:text-teal-400" to="/login">
              Login
            </NavLink>
          )}
        </div>
      ...
      {isOpen ? (
        <>
          <RiCloseFill onClick={toggleMenu} className="text-3xl text-white cursor-pointer z-50 fixed top-6 right-6" />
          <motion.div
            initial={{ x: "-100%" }}
            animate={{ x: 0 }}
            exit={{ x: "-100%" }}
            transition={{ duration: 0.4, ease: "easeInOut" }}
            className="fixed top-0 left-0 h-screen w-[100vw] sm:w-[50vw] bg-black/80 backdrop-blur-lg z-40 flex flex-col justify-center items-start p-8"
          >
            <h2 className="text-3xl text-white font-semibold mb-8">
              Ease<span className="text-teal-400">Learn</span>
            </h2>
            <NavLink className="text-2xl font-medium text-white mb-6 transition-colors duration-200 hover:text-teal-400" to="/" onClick={toggleMenu}>
              Home
            </NavLink>
            <NavLink className="text-2xl font-medium text-white mb-6 transition-colors duration-200 hover:text-teal-400" to="/courses" onClick={toggleMenu}>
              Courses
            </NavLink>
            <NavLink className="text-2xl font-medium text-white mb-6 transition-colors duration-200 hover:text-teal-400" to="/about" onClick={toggleMenu}>
              About
            </NavLink>
            {isAuth ? (
              <NavLink className="text-2xl font-medium text-white mb-6 transition-colors duration-200 hover:text-teal-400" to="/account" onClick={toggleMenu}>
                Account
              </NavLink>
            ) : (
              <NavLink className="text-2xl font-medium text-white mb-6 transition-colors duration-200 hover:text-teal-400" to="/login" onClick={toggleMenu}>
                Login
              </NavLink>
            )}
          </motion.div>
